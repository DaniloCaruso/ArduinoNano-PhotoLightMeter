...
void refresh() {
  ...
  display.print(F("lx:"));
  display.print(getLux(), 0);

  display.drawLine(72, linePos[0] - 1, 72, linePos[0] + 17, WHITE); // LINE DIVISOR
  display.setTextSize(1);
  display.setCursor(80, linePos[0]);
  display.print(F("EV: "));
    if (lux > 0) {
    display.print(EV, 0);
  } else {
...
// ND filter indicator
  if (ndIndex > 0) {
    ...
    display.setTextSize(1);
    display.setCursor(80, linePos[1]);
    display.print(F("ND"));
    //display.setCursor(110, linePos[0] + 10);
  ...
  display.setTextSize(1);
  display.setCursor(0, linePos[modeIndex]);
  display.print(F("*"));
